...
from rent_crawler.items import Property, Address, Prices, Details, MediaDetails
...
class QuintoAndarSpider(scrapy.Spider):
    ...
    data = '{{"filters":{{"map":{{"bounds_north":-23.50560423579402,"bounds_south":-23.595435764205977,' \
           '"bounds_east":-46.58431220239025,"bounds_west":-46.68230579760971,"center_lat":-23.55052,' \
           '"center_lng":-46.633309}},"availability":"any","occupancy":"any","sorting":{{"criteria":"relevance",' \
           '"order":"desc"}},"page_size":{page_size},"offset":{offset}}},"return":["id","coverImage","rent",' \
           '"totalCost","salePrice","iptuPlusCondominium","area","imageList","imageCaptionList","address",' \
           '"regionName","city","visitStatus","activeSpecialConditions","type","forRent","forSale","bedrooms",' \
           '"parkingSpaces","listingTags","yield","yieldStrategy"],"business_context":"RENT",' \
           '"user_id":"56307098-3c10-4139-8572-b1b425976775R"}}'
    headers = {
        'Accept': 'application/pclick_sale.v0+json'
    }
    custom_settings = {'DYNAMODB_PIPELINE_TABLE_NAME': 'quintoandar-items'}
    ...
    def start_requests(self):
        ...
        while page <= self.pages_to_crawl:
            json_data = self.data.format(page_size=PAGE_SIZE, offset=(page - 1) * PAGE_SIZE)
